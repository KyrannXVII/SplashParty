...
socket.on(
  "lancerPartie",
  (plateau, aQuiLeTour, liste_username, nbJoueur, roomId) => {
...
const deplacementPossible = async (event) => {
  ...
  function choixDirTrue() {
    direction = true; /*console.debug(`Tu as choisi T`)*/
  }
  function choixDirFalse() {
    direction = false; /*console.debug(`Tu as choisi F`)*/
  }
...
socket.on(
  "actualisePartie",
  async (plateau, aquiletour, liste_joueur_sans_couleur, action) => {
